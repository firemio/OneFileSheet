...
| 用途 | ポイント |
|---|---|
| AIエージェントとの共有シート | 「調査結果をこの表に書いて」「この表のTODOを処理して」— 人はブラウザ、エージェントはJSONで同じファイルを読み書き |
| LLM出力の受け皿テンプレ | 空シートを「この形式で出力して」と渡せば、返ってきたJSONが即・開ける表になる |
| 配れる自動ダッシュボード | cron / CI が sheet-data を書き換えるだけで「開けば最新」。Pages に置けばサーバーゼロの公開ダッシュボード |
| git が時系列DBになる | ベンチマーク履歴・カバレッジ推移を CI が追記。履歴・差分・巻き戻しは git が持つ |
| リポジトリ同梱の管理表 | API台帳・テスト表・チェックリストをPRでレビュー |
| 配って・記入して・回収する調査票 | 相手はブラウザで開くだけ。Excel不要 |
| .md が通じない相手に | 「Markdownビューア？入れてない」。.md は暗号、CSV は文字化け、Excel は無い。でもブラウザはある。ダブルクリックで表になり、1マス直して返ってくる |
